...
// appelé par choose-lang-logic.php
    ...
    $sql = "SELECT COUNT(code_language) 
            FROM users_languages 
            WHERE user_idl = :id_user 
            AND code_language = :cd_lang";
...
// extraction des niveaux de l'utilisateur par langues
    ...
    $sql ="SELECT code_language, level_user, lesson_user 
           FROM users_languages
           WHERE user_idl = :id_user";
...
// utilisé par choose-langues-logic.php
    ...
    $sql = "UPDATE users_languages 
            SET code_language = :cd_lang, current_lang = :cur_lang 
            WHERE user_idl = :id_user";
...
// utilisé par choose-langues-logic.php
    ...
    global $db;
    $sql = "UPDATE users_languages 
            SET current_lang = :cur_lang 
            WHERE user_idl = :id_user 
            AND code_language = :cd_lang";  // ancien
    ...
    $sql = "UPDATE users_languages 
            SET current_lang = :cur_lang 
            WHERE user_idl = :id_user 
            AND code_language = :cd_lang"; // nouveau
...
// mise à jour du nouveau pseudo
// appelé par: account-post.php
    function updatePseudo($id_user, $newPseudo) { 
    global $db;

    $sql = "UPDATE users 
            SET pseudo_user = :pseudo  
            WHERE id_user = :id_user";

    $upd = $db->prepare($sql);
    $upd->bindValue(":id_user", $id_user, PDO::PARAM_INT);
    $upd->bindValue(":pseudo", $newPseudo, PDO::PARAM_STR);
    $upd->execute();
    $upd->closeCursor();
} //
...
// appelé par: account-post.php
function updatePassword($id_user, $newPwd) { 
    global $db;
    ...
    $upd = $db->prepare($sql);
    $upd->bindValue(":id_user", $id_user, PDO::PARAM_INT);
    $upd->bindValue(":pwd", $newPwd, PDO::PARAM_STR);
    $upd->execute();
    $upd->closeCursor();
} //
